...
# ~5.5 times faster than stats::approx but devtools::check() gives warnings :(
# customApprox = function(x, fx, n.pts) {
#   xout = seq.int(x[1L], x[length(x)], length.out = n.pts)
#   yout = .Call(stats:::C_Approx, as.double(x), as.double(fx), xout, 1L, NA, NA, 0L)
#   return(list(x = xout, y = yout))
# }
#customApprox = function(x, fx, n.pts) stats::approx(x, y = fx, n = n.pts)
...
kurt = function(x, fx, n = length(x)) nth.cmomentS(x, fx, 4, n) / nth.cmomentS(x, fx, 2, n)^2
...
nth.momentS = function(x, fx, nth = 1, n = length(x)) simpson(x, x^nth * fx, n)
...
# simpson = function(x, fx, n = length(x)) {
#   ap = customApprox(x, fx, n.pts = 2*n + 1)
#   return(simpsonC(ap$x, ap$y))
# }
